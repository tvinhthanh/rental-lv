"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.HandoverService = void 0;
const common_1 = require("@nestjs/common");
const prisma_service_1 = require("../../prisma/prisma.service");
const audit_log_service_1 = require("../audit-log/audit-log.service");
let HandoverService = class HandoverService {
    constructor(prisma, audit) {
        this.prisma = prisma;
        this.audit = audit;
    }
    async findByBooking(bookingId) {
        return this.prisma.handover.findUnique({
            where: { bookingId },
            include: { booking: true }
        });
    }
    async create(dto, actorId) {
        var _a, _b;
        const booking = await this.prisma.booking.findUnique({
            where: { id: dto.bookingId }
        });
        if (!booking)
            throw new common_1.NotFoundException('Booking not found');
        const existing = await this.prisma.handover.findUnique({
            where: { bookingId: dto.bookingId }
        });
        if (existing) {
            const updated = await this.prisma.handover.update({
                where: { bookingId: dto.bookingId },
                data: Object.assign(Object.assign({}, dto), { photoUrls: (_a = dto.photoUrls) !== null && _a !== void 0 ? _a : existing.photoUrls })
            });
            await this.audit.log(actorId !== null && actorId !== void 0 ? actorId : null, 'UPDATE', 'Handover', existing.id, {
                before: existing,
                after: updated
            });
            return updated;
        }
        const handover = await this.prisma.handover.create({
            data: Object.assign(Object.assign({}, dto), { photoUrls: (_b = dto.photoUrls) !== null && _b !== void 0 ? _b : [] })
        });
        await this.audit.log(actorId !== null && actorId !== void 0 ? actorId : null, 'CREATE', 'Handover', handover.id, handover);
        return handover;
    }
};
exports.HandoverService = HandoverService;
exports.HandoverService = HandoverService = __decorate([
    (0, common_1.Injectable)(),
    __metadata("design:paramtypes", [prisma_service_1.PrismaService,
        audit_log_service_1.AuditLogService])
], HandoverService);
//# sourceMappingURL=handover.service.js.map